...
const send = (obj: any) =>
  window.top?.postMessage({ ...obj, context: params.context }, "*");
const receive = (type: string, cb: (e: any) => void) =>
  window.addEventListener("message", (e) => e.data.type === type && cb(e.data));
...
): Promise<monaco.editor.IStandaloneThemeData | undefined> => {
  return themeName
    ? fetch("/themes/" + themeName + ".json").then((res) => res.json())
...
const changeBackground = async (color: string, theme?: string) => {
  const fixedColor = color.startsWith("#")
    ? color
    : color === "transparent"
    ? "#00000000"
    : "#" + color;
  ...
  const customBGTheme = {
    base:
      getBuiltinTheme(theme) ??
      getBuiltinTheme(customTheme?.base) ??
      ("vs" as any),
...
if (params.javascriptDefaults) {
  monaco.languages.typescript.javascriptDefaults.setDiagnosticsOptions({
    noSemanticValidation:
      params.javascriptDefaultsNoSemanticValidation === "true",
...
if (params.typescriptDefaults) {
  monaco.languages.typescript.typescriptDefaults.setDiagnosticsOptions({
    noSemanticValidation:
      params.typescriptDefaultsNoSemanticValidation === "true",
